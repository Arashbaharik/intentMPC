...
% Run MPC
L     = 1; %number of realization
for l  = 1:L  %L=10 for the nominal realization
    ...
    for k=1:K    %run MPC for k=1:K
        ...
        YY(:,k+1)   = [YY(1,k)+t_e*UU1(1,k)*cos(YY(3,k));YY(2,k)+t_e*UU1(1,k)*sin(YY(3,k));YY(3,k)+UU1(2,k)+0.0001*((-1)^l*floor(l/2)*(L<=20)+(rand-0.5)*(20)*(L>20))]; %run the intruder with a possible uncertainty
